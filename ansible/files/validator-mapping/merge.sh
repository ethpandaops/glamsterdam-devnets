...
# underlying keys.
#
# PATCHED COPY of /apps/validator-mapping/merge.sh from
# ethpandaops/ethereum-genesis-generator:6.2.0. The upstream script hands the
# mapping and the segments to jq via --argjson, which blows past the Linux
# per-argument limit (MAX_ARG_STRLEN, 128 KiB) once the mapping gets large:
# devnet-9 shuffles 4M validators, so metadata/validator_names.yaml is ~2.8 MB
# of YAML / ~4 MB of JSON and the merge died with
#   /apps/validator-mapping/merge.sh: line 75: /usr/local/bin/jq: Argument list too long
# This copy stages both inputs in temp files and reads them with --slurpfile
# instead; the jq program itself is unchanged. It is bind-mounted over the
# image's copy via eth_inventory_web_merge_image (see
# inventories/devnet-9/group_vars/all/all.yaml). Upstreamed as
# ethpandaops/ethereum-genesis-generator#318; delete this file and the override
# once an image carrying that fix is pinned.
#
...
# Both jq inputs are staged as files: multi-MB mappings do not fit in a single
# execve argument, so --argjson is not an option at devnet scale.
work_dir="$(mktemp -d)"
...
jq -rn \
  ...
  # --slurpfile wraps each file in a one-element array of documents.
